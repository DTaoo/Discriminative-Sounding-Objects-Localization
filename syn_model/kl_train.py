...
def location_model_train(model, av_model_fix, data_loader, optimizer, criterion_location, criterion_class, epoch, obj_rep):
    ...
    for i, data in enumerate(data_loader, 0):
        ...
        model.zero_grad()
        av_outputs, _, _, sounding_objects, sounding_object_local = model(image_data, audio_data, obj_rep_cuda, epoch>15)
...
#    loss_func_BCE_class = torch.nn.BCELoss(reduce=True)
    ...
    for e in range(0, args.epoch):
        ...
        if e % 1 == 0:
            PATH = 'ckpt/synkl/audioset_pretrain_%03d_%.3f.pth' % (e, train_location_acc)
