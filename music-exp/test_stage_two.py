...
def visualize_model(model, av_model_fix, data_loader, obj_rep):
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader, 0):
            if i == 1 : break
            audio_data, posi_img_data, nega_img_data, boxs, path = data
            ...
            imgs = posi_img_data.cpu().numpy()
            visualize(posi_img_data, obj_localization, i)
            ...
            for j in range(obj_localization.shape[0]):
                results[path[j][:-4]] = obj_localization[j]
                avmaps[path[j][:-4]] = snd_localization[j]
                box = boxs[j]
                box = np.array(boxs[j]) * 224
...
def visualize(image, avmaps, idx):
    ...
    for i in range(image.shape[0]):
        ...
        for k in range(avmaps.shape[1]):
            avmap = avmaps[i, k]
            avmap = cv2.resize(avmap, (224, 224))
            avmap = avmap * 255 / np.max(avmap) * 0.9
            avmap = cv2.applyColorMap(avmap.astype(np.uint8), cv2.COLORMAP_JET)
            new = 0.6 * img + 0.4 * (avmap[:, :, ::-1] / 255)
            plt.imsave('visualize/%d_%d_%d.jpg' % (idx, i, k), new)
