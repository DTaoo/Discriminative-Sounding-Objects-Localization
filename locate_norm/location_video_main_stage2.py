...
def location_model_train(model, av_model_fix, data_loader, optimizer, criterion_location, criterion_class, epoch, obj_rep):
    ...
    for i, data in enumerate(data_loader, 0):
        ...
        model.zero_grad()
        av_outputs, _, _, sounding_objects, sounding_object_local = model(image_data, audio_data, obj_rep_cuda, epoch>100)
...
def visualize(images, cams, boxs, e):
    ...
    for i in range(images.shape[0]):
        ...
        if i % 2 == 0 and boxs is not None:
            for box in boxs[i//2]:
                box = box * 224
                cv2.rectangle(cam, (int(box[0]), int(box[1])), (int(box[0]+box[2]), 
                                        int(box[1]+box[3])), (0, 255, 0), 2)
        cam = cam[:, :, ::-1] / 255
        plt.imsave('vis/img_'+str(e)+'_'+str(i)+cor+'.jpg', 0.4*cam+0.6*image)
...
def visualize_cam(images, cams, scores, corr, e):
    ...
    for i in range(images.shape[0]):
        ...
        for k in range(2):
            ...
            cam = cv2.applyColorMap(cam.astype(np.uint8), cv2.COLORMAP_JET)
            cam = cam[:, :, ::-1] / 255
            plt.imsave('vis/img_'+str(e)+'_'+str(i)+'_'+str(idx[k])+'.jpg', 0.4*cam+0.6*image)
...
def visualize_object(images, cams, ks, e):
    ...
    for i in range(images.shape[0]):
        ...
        for k in ks[i//2]:
            ...
            cam = cam[:, :, ::-1] / 255
            plt.imsave('vis/img_'+str(e)+'_'+str(i)+'_'+str(int(k[-1]))+'.jpg', 0.4*cam+0.6*image)
...
def main():
    ...
    # fixed model
    visual_backbone_fix = resnet18(pretrained=True, modal='vision')
...
#    loss_func_BCE_class = torch.nn.BCELoss(reduce=True)
    loss_func_BCE_class = torch.nn.KLDivLoss(reduce=True)
...
#                                                  loss_func_BCE_location, loss_func_BCE_class, e, obj_rep)
        ...
        if e % 1 == 0:
            PATH = 'ckpt/synorm/audioset_pretrain_%03d_%.3f.pth' % (e, train_location_acc)
