...
#                    idx_margin+1, int(count_framesN)-idx_margin)
                center_frameN = random.randint(0, int(count_framesN)-1)
            else:
...
#                        '{:06d}.jpg'.format(center_frameN + idx_offset)))
            for i in range(self.num_frames):
                path_frames[n] = (os.path.join(path_frameN, 'frame_'+str(center_frameN).zfill(3)))
...
#                center_timeN = (center_frames[n] - 0.5) / self.fps
                audios[n] = self._load_audio(path_audios[n], center_frames[n])
